...
public class Schedules implements java.io.Serializable{
    ...
    public void setStart_time(){this.start_time = start_time;}
    public String getEnd_time(){return end_time;}
    public void setEnd_time(){this.end_time = end_time;}
    ...
    public static Schedules getScheduleById(int activityid, int seniorid, int caretakerid) {
        ...
        try (Connection conn = DriverManager.getConnection("jdbc:derby://localhost:1527/SeniorCareCoordination", "scc", "scc");
             PreparedStatement stmt = conn.prepareStatement("SELECT s.activityID, s.seniorID, s.caretakerID, " + "a.activityName, sen.seniorName, c.caretakerName " + "FROM SCHEDULES s " + "JOIN Activities a ON s.activityID = a.activityID " + "JOIN Seniors sen ON s.seniorID = sen.seniorID " + "LEFT JOIN Caretakers c ON s.caretakerID = c.caretakerID " + "WHERE s.activityID = ? AND s.seniorID = ?")) {
      ...
      public boolean update() {
        try (Connection conn = DriverManager.getConnection("jdbc:derby://localhost:1527/SeniorCareCoordination", "scc", "scc");
             PreparedStatement stmt = conn.prepareStatement("UPDATE SCHEDULE SET START_TIME = ?, END_TIME = ?, DATE = ?, STATUS = ? WHERE ID = ?")) {
    ...
    public static List<Schedules> getAllSchedules() {
        ...
        try (Connection conn = DriverManager.getConnection("jdbc:derby://localhost:1527/SeniorCareCoordination", "scc", "scc");
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM SCHEDULES");
